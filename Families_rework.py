...
#        self.family_id = pedigree
    ...
    def get_couple_by_sample_id(self, sample_id):
        for couple in self.couples:
            if sample_id in couple:
                return couple
        return sample_id

    def couplize_generation(self, generation):
        couples = set()
        for sample in generation:
            couples.add(self.get_couple_by_sample_id(sample))
        return [couples]
    
    def get_gen_by_sample_id(self, sample_id):
        for i in range(len(self.generations)):
            if sample_id in self.generations[i]:
                return i
        return -1

    def get_siblings(self, sample_id):
        for sib_group in self.siblings_by_mom:
            print ()
            if sample_id in sib_group:
                return si_group
        return [sample_id]

    def get_sample_by_id(self, sample_id):
        for sample in self.samples:
            if sample[1] == sample_id:
                return sample
        return None

    def pretty_print(self):
        if len(self.generations) != 3:
            print ("pretty-printing of more or less than 3-generation pedigrees is not yet supported")
            sys.exit(1)

        last_gen_subfamilies = []
        for kid in self.generations[-1]:
            last_gen_subfamilies.append(self.get_siblings(kid))
            last_gen_subfamilies = list(set(last_gen_subfamilies))
        print (last_gen_subfamilies)


        for subfam in last_gen_subfamilies:
            spacer2 = "  "
            for i in range(len(subfam)):
                spacer2 += "|       " 
            
            subfam_str = spacer2+"\n"
            subfam_str += "    ".join(subfam) + "  "
            print (sub_fam_str)
            sys.exit()
